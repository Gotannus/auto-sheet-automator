...
export const Route = createFileRoute("/_authenticated/dashboard")({
  ...
  errorComponent: ({ error }) => (
    <div className="p-6">Erro: {error.message}</div>
  ),
});
...
function DashboardPage() {
  ...
  const dashQuery = useQuery({
    queryKey: ["dash", productId, year, month],
    queryFn: () => fetchDash({ data: { product_id: productId!, year, month } }),
    enabled: !!productId,
  });
        ...
        <Card>
          <CardContent className="p-8 text-center space-y-3">
            ...
            <p className="text-muted-foreground">
              Cadastre o primeiro produto com nome + SRC da Celetus para começar.
            </p>
            <Button asChild>
              <Link to="/products">Cadastrar produto</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <header className="flex flex-col md:flex-row md:items-end gap-3 md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {MONTHS[month - 1]} {year} — métricas por dia, alimentadas via webhook da Celetus.
          </p>
        </div>
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-0 overflow-x-auto">
          <DailyTable productId={productId} days={data.days} />
        </CardContent>
      </Card>
    </div>
  );
...
function DailyTable({ productId, days }: { productId: string; days: Awaited<ReturnType<typeof getDashboard>>["days"] }) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {days.map((d) => (
          <DailyRow key={d.date} productId={productId} day={d} />
        ))}
      </TableBody>
    </Table>
  );
...
function DailyRow({
  ...
  const mut = useMutation({
    mutationFn: (patch: Record<string, unknown>) =>
      save({ data: { product_id: productId, date: day.date, ...patch } as never }),
  ...
  return (
    <TableRow>
      ...
      <TableCell className="text-right">{day.invest_final ? fmtBRL(day.invest_final) : "-"}</TableCell>
      ...
      <TableCell className="text-right">
        <NumCell value={clicks} onChange={setClicks} integer onCommit={(v) => mut.mutate({ clicks: v })} />
      </TableCell>
      <TableCell className="text-right">
        <NumCell value={checkouts} onChange={setCheckouts} integer onCommit={(v) => mut.mutate({ checkouts: v })} />
      </TableCell>
      <TableCell className="text-right">
        <NumCell value={impressions} onChange={setImpressions} integer onCommit={(v) => mut.mutate({ impressions: v })} />
      </TableCell>
      <TableCell>
        <Textarea
          rows={1}
