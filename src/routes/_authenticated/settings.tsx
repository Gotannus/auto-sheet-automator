...
export const Route = createFileRoute("/_authenticated/settings")({
  head: () => ({ meta: [{ title: "Configurações — Painel Celetus" }] }),
...
function SettingsPage() {
  ...
  const mut = useMutation({
    mutationFn: () =>
      save({ data: { year, tax_rate: Number(taxPct.replace(",", ".")) / 100 } }),
    onSuccess: () => {
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-xl">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Mesmas opções da aba CONFIG da planilha original.
        </p>
      </header>
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Aplicada como Invest. Final = Invest. Manual × (1 + taxa). Use 0 se o investimento já vier com imposto.
            </p>
          </div>
